    ...
    * */
    ...
    public static String  getResult(HttpRequestBase target){
        ...
        try {
            ...
            e.printStackTrace();
            /*服务器连接失败*/
            return JSON.toJSONString(ResponseServer.error(EnumService.SERVER_CONNECT_ERROR));
        } catch (SocketTimeoutException e) {
            e.printStackTrace();
            /*服务器连接超时*/
            return JSON.toJSONString(ResponseServer.error(EnumService.SERVER_BUSYNESS));
        } catch (IOException e) {
            e.printStackTrace();
            /*连接失败*/
            return JSON.toJSONString(ResponseServer.error(EnumService.SERVER_ERROR));
        }finally {
            if(response != null){
                try {
                    response.close();
